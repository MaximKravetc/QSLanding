...
namespace Shelby\Service\OAuth2;
...
abstract class ServerAbstract {
	 ...
	 *   values contained in self::AUTH_RESPONSE_TYPE_REGEXP.
	 *
	 * @return boolean
	 ...
	 *
	 * Required for self::GRANT_TYPE_AUTH_CODE.
	 *
	 * @param $code
	 ...
	 *
	 * Required for self::GRANT_TYPE_USER_CREDENTIALS.
	 *
	 * @param $client_id
	 *   Client identifier to be check with.
	 * @param $username
	 ...
	 * on a client, if you need that.
	 *
	 * Required for self::GRANT_TYPE_ASSERTION.
	 *
	 * @param $client_id
	 *   Client identifier to be check with.
	 ...
	 *
	 * Required for self::GRANT_TYPE_REFRESH_TOKEN.
	 *
	 * @param $refresh_token
	 ...
	 *
	 * Required for self::GRANT_TYPE_NONE.
	 *
	 * @ingroup oauth2_section_4
	 * @param $client_id
	 * @return bool
	...
	public function verifyAccessToken($scope = NULL, $exit_not_present = TRUE, $exit_invalid = TRUE, $exit_expired = TRUE, $exit_scope = TRUE, $realm = NULL) {
		...
		if ($token === NULL) {
			if ($exit_invalid) {
				$this->errorWWWAuthenticateResponseHeader(self::HTTP_UNAUTHORIZED, $realm, self::ERROR_INVALID_TOKEN, 'The access token provided is invalid.', NULL, $scope);
			} else {
				return false;
			}
		}
		...
		if (isset($token['expires']) && time() > $token['expires']) {
			if ($exit_expired) {
				$this->errorWWWAuthenticateResponseHeader(self::HTTP_UNAUTHORIZED, $realm, self::ERROR_EXPIRED_TOKEN, 'The access token provided has expired.', NULL, $scope);
			} else {
				return false;
			}
		}
		...
		if ($scope && (!isset($token['scope']) || !$token['scope'] || !$this->checkScope($scope, $token['scope']))) {
			if ($exit_scope) {
				$this->errorWWWAuthenticateResponseHeader(self::HTTP_FORBIDDEN, $realm, self::ERROR_INSUFFICIENT_SCOPE, 'The request requires higher privileges than provided by the access token.', NULL, $scope);
			} else {
				return false;
			}
		}
	...
	private function getAccessTokenParams() {
		...
		if ($auth_header !== FALSE) {
			...
			if (isset($_GET[self::TOKEN_PARAM_NAME]) || isset($_POST[self::TOKEN_PARAM_NAME]))
				$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST, 'Auth token found in GET or POST when token present in header');

			$auth_header = trim($auth_header);

			// Make sure it's Token authorization
			if (strcmp(substr($auth_header, 0, 5), "OAuth ") !== 0)
				$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST, 'Auth header found that doesn\'t start with "OAuth"');
		...
		if (isset($_GET[self::TOKEN_PARAM_NAME])) {
			if (isset($_POST[self::TOKEN_PARAM_NAME])) // Both GET and POST are not allowed
				$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST, 'Only send the token in GET or POST, not both');
	...
	public function grantAccessToken() {
		...
		if (!$input["grant_type"])
			$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST, 'Invalid grant_type parameter or parameter missing');

		// Make sure we've implemented the requested grant type
		if (!in_array($input["grant_type"], $this->getSupportedGrantTypes()))
			$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_UNSUPPORTED_GRANT_TYPE);

		// Authorize the client
		$client = $this->getClientCredentials();

		if ($this->checkClientCredentials($client[0], $client[1]) === FALSE)
			$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_CLIENT);

		if (!$this->checkRestrictedGrantType($client[0], $input["grant_type"]))
			$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_UNAUTHORIZED_CLIENT);

		// Do the granting
		switch ($input["grant_type"]) {
			case self::GRANT_TYPE_AUTH_CODE:
				if (!$input["code"] || !$input["redirect_uri"])
					$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST);
				...
				if ($stored === NULL || (strcasecmp(substr($input["redirect_uri"], 0, strlen($stored["redirect_uri"])), $stored["redirect_uri"]) !== 0) || $client[0] != $stored["client_id"])
					$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_GRANT);

				if ($stored["expires"] < time())
					$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_EXPIRED_TOKEN);

				// Scope is optional and may not be passed, but we need it to know token access level
				if (empty($input['scope']) && !empty($stored['scope'])) {
			...
			case self::GRANT_TYPE_USER_CREDENTIALS:
				if (!$input["username"] || !$input["password"])
					$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST, 'Missing parameters. "username" and "password" required');
			...
			case self::GRANT_TYPE_REFRESH_TOKEN:
				if (!$input["refresh_token"])
					$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST, 'No "refresh_token" parameter found');
			...
			case self::GRANT_TYPE_NONE:
				...
				break;
			default:
				throw new \Exception('Unknown grant type - ' . $input['grant_type']);
		}

		// Check scope, if provided
		if ($input["scope"] && (!is_array($stored) || !isset($stored["scope"]) || !$this->checkScope($input["scope"], $stored["scope"])))
			$this->errorJsonResponse(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_SCOPE);
	...
	public function getAuthorizeParams() {
		$filters = array(
			...
			"response_type" => array("filter" => FILTER_VALIDATE_REGEXP, "options" => array("regexp" => self::AUTH_RESPONSE_TYPE_REGEXP), "flags" => FILTER_REQUIRE_SCALAR),
			"redirect_uri" => array("filter" => FILTER_SANITIZE_URL),
		...
		if (!$input["client_id"]) {
			...
			$this->errorJsonResponse(self::HTTP_FOUND, self::ERROR_INVALID_CLIENT); // We don't have a good URI to use
		}
		...
		if (!$redirect_uri && !$input["redirect_uri"])
			$this->errorJsonResponse(self::HTTP_FOUND, self::ERROR_INVALID_REQUEST);
		...
		if (!$input["response_type"])
			$this->errorDoRedirectUriCallback($input["redirect_uri"], self::ERROR_INVALID_REQUEST, 'Invalid response type.', NULL, $input["state"]);
	...
	public function finishClientAuthorization($is_authorized, $params = array()) {
		...
		if ($is_authorized === FALSE) {
			...
			if ($params['response_type'] == self::AUTH_RESPONSE_TYPE_AUTH_CODE || $params['response_type'] == self::AUTH_RESPONSE_TYPE_CODE_AND_TOKEN)
				$result["query"]["code"] = $this->createAuthCode($params['client_id'], $params['redirect_uri'], $params['scope']);

			if ($params['response_type'] == self::AUTH_RESPONSE_TYPE_ACCESS_TOKEN || $params['response_type'] == self::AUTH_RESPONSE_TYPE_CODE_AND_TOKEN)
				$result["fragment"] = $this->createAccessToken($params['client_id'], $params['scope']);
		}
	...
	protected function createAccessToken($client_id, $scope = NULL) {
		$token = array(
			...
			"expires_in" => $this->getVariable('access_token_lifetime', self::DEFAULT_ACCESS_TOKEN_LIFETIME),
			"scope" => $scope
		);

		$this->setAccessToken($token["access_token"], $client_id, time() + $this->getVariable('access_token_lifetime', self::DEFAULT_ACCESS_TOKEN_LIFETIME), $scope);

		// Issue a refresh token also, if we support them
		if (in_array(self::GRANT_TYPE_REFRESH_TOKEN, $this->getSupportedGrantTypes())) {
			$token["refresh_token"] = $this->genAccessToken();
			$this->setRefreshToken($token["refresh_token"], $client_id, time() + $this->getVariable('refresh_token_lifetime', self::DEFAULT_REFRESH_TOKEN_LIFETIME), $scope);
	...
	private function createAuthCode($client_id, $redirect_uri, $scope = NULL) {
		...
		$this->setAuthCode($code, $client_id, $redirect_uri, time() + $this->getVariable('auth_code_lifetime', self::DEFAULT_AUTH_CODE_LIFETIME), $scope);
		return $code;
	}
