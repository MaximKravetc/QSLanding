...
namespace Shelby\Service\Drupal;
...
class Password {
	 ...
	 * @param $password
	 *   A plain-text password
	 * @param $account_pass_hash
	 *   A user password HASH in database to compare to
	 *
	 * @return bool
	 *   TRUE or FALSE.
	 */
	public static function user_check_password($password, $account_pass_hash) {
		if (substr($account_pass_hash, 0, 2) == 'U$') {
			// This may be an updated password from user_update_7000(). Such hashes
			// have 'U' added as the first character and need an extra md5().
			$stored_hash = substr($account_pass_hash, 1);
			$password = md5($password);
		} else {
			$stored_hash = $account_pass_hash;
		}
